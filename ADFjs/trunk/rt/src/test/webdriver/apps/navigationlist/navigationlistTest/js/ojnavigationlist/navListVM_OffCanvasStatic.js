    ...
    'ojs/ojoffcanvas'
        ...
        function (oj, ko, $) // this callback gets executed when all required modules are loaded
        {
            ...
            function ViewModel() {
                ...
                this.toggleButton = function (edge, clear)
                {
                    this.checkedState[edge](this.checkedState[edge]().length ? [] : [edge])
                };

                // An object that maps edges to offcanvas objects
                this.edgeMap =
                        {
                            "start": {
                                "edge": "start",
                ...
                self.closeDrawer = function (drawer) {
                    oj.Logger.info("Close Drawer")
                    self.toggleButton(drawer.edge);
                    return oj.OffcanvasUtils.close(drawer);
                };
                
                    // toggle show/hide offcanvas
                this.toggleDrawer = function (model, event)
                {
                    var edge = event.currentTarget.id;
                    oj.Logger.info("togglDrawer:  current tartget Id: " + edge);
                    var drawer = self.edgeMap[edge];
                ...
                $("#startDrawer").on("ojclose",
                        function ()
                        {
                            oj.Logger.info("ojClose for Drawer")
                ...
                self.expandedValueArray = ko.observableArray(['save', 'playback', 'play']);
                self.rootLabelValue = ko.observable('');
                self.expandedComboboxValue = ko.observableArray(['save', 'playback', 'play']);
