...
define(['ojs/ojcore',
    ...
    'ojs/ojgauge'
        ...
        function (oj, ko, $) // this callback gets executed when all required modules are loaded
        {
            ...
            $.getJSON("../js/ojnavigationlist/json/root.json",
                    function (data)
                    {
                        function ViewModel() {
                            ...
                            self.currentRendererReturnType = ko.observable('dom');
                               self.navListItemRenderer_str = function (itemContext) {
                                oj.Logger.info("renderer start: ############");
                                var obj = itemContext.data;
                                for (var property in obj) {
                                    if (obj.hasOwnProperty(property)) {
                                        oj.Logger.info("property: " + property);
                                        if (property == 'attr1') {
                                            for (var p in property) {
                                                if (property.hasOwnProperty(p)) {
                                                    oj.Logger.info("   sub property: " + p);
                                                    oj.Logger.info("    sub property value: " + property[p]);
                                                }
                                            }
                                        }
                                        else
                                            oj.Logger.info("value: " + obj[property]);
                                    }
                                }
                                oj.Logger.info("renderer end: ############");

                                return itemContext.data.title;
                            }
