...
public class CookbookMultipleMsgsOnInputNumber extends OJetBase {
    public CookbookMultipleMsgsOnInputNumber() {
    ...
    /*
     *  Test how inputNumber component respond when multiple messages of different severities are set using the messages options
    ...
    public void testAddMultipleMessages() throws Exception {
        ...
        waitForElementVisible("id=spinnercontrol");

        //initially no msg exists on spinnercontrol
        //move focus on spinnercontrol and verify no notewindow exists

        Actions actions = new Actions(getWebDriver());
        WebElement text = getElement("id=spinnercontrol");
        ...
        Assert.assertNull(msg, "initially no message exists on inputNumber");
        ...
        //Move focus on inputNumber field
        ...
        Assert.assertFalse(hasValidationErr, "oj-invalid is NOT applied to inputNumber");
        hasValidationErr = classes.indexOf("oj-warning") > -1;
        Assert.assertFalse(hasValidationErr, "oj-warning is NOT applied to inputNumber");
        ...
        classes = parentElem.getAttribute("class");
        hasValidationErr = classes.indexOf("oj-warning") > -1;
        Assert.assertTrue(hasValidationErr, "oj-warning is applied to inputNumber");
        ...
        classes = parentElem.getAttribute("class");
        hasValidationErr = classes.indexOf("oj-invalid") > -1;
        Assert.assertTrue(hasValidationErr, "oj-invalid is applied to inputNumber");
        ...
        //unselect all msg types and verify that uinputNumber has no msges
        actions.sendKeys(Keys.ESCAPE).build().perform();
        fatalBtn.click();
        errorBtn.click();
        warningBtn.click();
        ...
        actions.moveToElement(text).click().perform();
        msg = getMessagingContentNodeBySubId(text, "oj-messaging-popup-message-summary", 0);
        Assert.assertNull(msg, "no message exists on inputNumber");
    }
    ...
    public void testRemoveMessageTypesOneByOne() throws Exception {
        ...
        //Deselect fatal msg
        fatalBtn.click();
        //Move focus on inputNumber and verify that 4 msg types are displayed in order with correct icons
