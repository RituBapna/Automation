...
public class CookbookMultipleMsgsOnSelect extends OJetBase {
    public CookbookMultipleMsgsOnSelect() {
    ...
    /*
        *  Test how select component respond when multiple messages of different severities are set using the messages options
    ...
    public void testAddMultipleMessages() throws Exception {
        ...
        //move focus on select and verify no notewindow exists

        Actions actions = new Actions(getWebDriver());
        WebElement selectChosen = getElement("{\"element\":\"#select\",\"subId\":\"oj-select-chosen\"}");
        WebElement text = getParentElement(selectChosen);
        ...
        Assert.assertNull(msg, "initially no message exists on select");
        ...
        Assert.assertFalse(hasValidationErr, "oj-invalid is NOT applied to select");
        hasValidationErr = classes.indexOf("oj-warning") > -1;
        Assert.assertFalse(hasValidationErr, "oj-warning is NOT applied to select");
        ...
        classes = parentElem.getAttribute("class");
        hasValidationErr = classes.indexOf("oj-warning") > -1;
        Assert.assertTrue(hasValidationErr, "oj-warning is applied to select");
        ...
        classes = parentElem.getAttribute("class");
        hasValidationErr = classes.indexOf("oj-invalid") > -1;
        Assert.assertTrue(hasValidationErr, "oj-invalid is applied to select");
        ...
        //unselect all msg types and verify that uselect has no msges
        actions.sendKeys(Keys.ESCAPE).build().perform();
        fatalBtn.click();
        errorBtn.click();
        warningBtn.click();
        ...
        actions.moveToElement(text).click().perform();
        msg = getMessagingContentNodeBySubId(text, "oj-messaging-popup-message-summary", 0);
        Assert.assertNull(msg, "no message exists on select");
    }
    ...
    public void testRemoveMessageTypesOneByOne() throws Exception {
        ...
        //Deselect fatal msg
        fatalBtn.click();
        //Move focus on select and verify that 4 msg types are displayed in order with correct icons
