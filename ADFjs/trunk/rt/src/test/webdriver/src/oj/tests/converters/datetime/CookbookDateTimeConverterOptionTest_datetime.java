...
public class CookbookDateTimeConverterOptionTest_datetime extends OJetBase {
    ...
    public CookbookDateTimeConverterOptionTest_datetime() {
    ...
    public void testInvalidMonth() throws Exception {
        ...
        WebElement input2 =
          ...
          Actions actions = new Actions(getWebDriver());
        actions.sendKeys(input, Keys.ESCAPE);
        actions.sendKeys(input, "13/30/00 10 pm");
        ...
        actions.sendKeys(input, Keys.ESCAPE);
        actions.sendKeys(input, Keys.HOME, Keys.chord(Keys.SHIFT, Keys.END), Keys.DELETE); //delete the existing value
        actions.sendKeys(input, "12/30/00 10 pm"); //enter the new value
        actions.moveToElement(input2).click().perform(); // move focus to next input date field
        //Verify that error is removed
        classes = parentElem.getAttribute("class");
        hasValidationErr = classes.indexOf("oj-invalid") > -1;
    ...
    public void testInvalidHour() throws Exception {
        ...
        Actions actions = new Actions(getWebDriver());
        actions.sendKeys(input, Keys.ESCAPE);
        actions.sendKeys(input, "02/29/2016 14:00");
        ...
        actions.sendKeys(input, Keys.ESCAPE);
        actions.sendKeys(input, Keys.HOME, Keys.chord(Keys.SHIFT, Keys.END), Keys.DELETE); //delete the existing value
        actions.sendKeys(input, "02/29/2016 12:00 PM"); //enter the new value
        actions.moveToElement(input2).click().perform(); // move focus to next input date field
        //Verify that error is removed
        classes = parentElem.getAttribute("class");
        hasValidationErr = classes.indexOf("oj-invalid") > -1;
    ...
    public void testLenientParsing2() throws Exception {
        ...
        Actions actions = new Actions(getWebDriver());
        actions.sendKeys(input, Keys.ESCAPE);
        actions.sendKeys(input, "2016, 30 Nov 10?:45 pm");
